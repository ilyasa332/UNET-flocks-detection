...
def create_early_image_2(files, file, num_past, minuts, box, sz):
    """
    ...
    """
    time = datetime.strptime(re.search('--(.*)_VRADH', os.path.basename(file)).group(1).replace('-', ' '),
                             '%Y%m%d %H%M%S')
    index = files.index(file)
    time_prev = datetime.strptime(
        re.search('--(.*)_VRADH', os.path.basename(files[index - 1])).group(1).replace('-', ' '),
        '%Y%m%d %H%M%S')
    ...
    if delta < minuts:
        ...
        if num_past > 1:
            time_prev_2 = datetime.strptime(
                re.search('--(.*)_VRADH', os.path.basename(files[index - 2])).group(1).replace('-', ' '),
                '%Y%m%d %H%M%S')
